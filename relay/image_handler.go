...
func ImageHelper(c *gin.Context, info *relaycommon.RelayInfo) (newAPIError *types.NewAPIError) {
	_, _, newAPIError = ExecuteImage(c, info, true)
	return newAPIError
}

// ExecuteImage runs an image request. Async task workers disable billing until
// the successful task result has been persisted with a CAS transition.
func ExecuteImage(c *gin.Context, info *relaycommon.RelayInfo, settleBilling bool) (*dto.Usage, []string, *types.NewAPIError) {
	...
	return imageUsage, logContent, nil
}
...
func shouldPassThroughImageRequest(info *relaycommon.RelayInfo) bool {
	...
	if info.ApiType == constant.APITypeGemini && model_setting.IsGeminiModelSupportImagine(info.UpstreamModelName) {
		return false
	}
